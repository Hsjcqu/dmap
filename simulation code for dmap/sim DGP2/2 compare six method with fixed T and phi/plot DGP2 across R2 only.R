...
MSPE_R2a = read.table("MSPE across R2(N=2^13, J=2^6, p=15, T=5)(Case 1).txt", header=TRUE)
MSPE_R2b = read.table("MSPE across R2(N=2^13, J=2^6, p=15, T=5)(Case 2).txt", header=TRUE)
RISK_R2a = read.table("RISK across R2(N=2^13, J=2^6, p=15, T=5)(Case 1).txt", header=TRUE)
RISK_R2b = read.table("RISK across R2(N=2^13, J=2^6, p=15, T=5)(Case 2).txt", header=TRUE)
...
rect(0.48, 0.04, 0.52, 0.12, lty=1, border ="gray")  # same as xlim and ylim
...
## Case 1, Risk_R2 ##
plot(R2, RISK_R2a[,1], ylim=c(0, 1.5), type="n", #range(RISK_R2a)[1]
...
## Case 2, Risk_R2 ##
plot(R2, RISK_R2b[,1], ylim=c(0, 1.5), type="n",  #range(RISK_R2b)[1]
      xlab=expression(R^2), ylab="RISK")
for(i in 1:length(name_method)){
...
par(mar=c(3,3,1,1), font=3, fig = c(0.07, 0.31, 0.57, 0.82), new = T)  
plot(R2, log(MSPE_R2[,1], 10), ylim=c(0.07, 0.095), type="n", #range(log(MSPE_R2, 10))[1]
