...
Time_N = read.table("CPU time across N(J=2^6, R2=0.5, p=15)DGP2.txt", header=TRUE)
N_vec = 2^(12:16)
Time_J = read.table("CPU time across J(N=2^13, R2=0.5, p=15)DGP2.txt", header=TRUE)
J_vec = 2^(2:7)
Time_p = read.table("CPU time across p(N=2^13, J=2^6, R2=0.5)DGP2.txt", header=TRUE)
p_vec=c(5, 10, 15, 20, 30)
Time_R2 = read.table("CPU time across R2(N=2^13, J=2^6, p=15)DGP2.txt", header=TRUE)
R2_vec=c(0.1, 0.3, 0.5, 0.7, 0.9)


name_method = c(expression(paste('DMAP-SA( ', italic(phi),'=2 )')), 
                 expression(paste('DMAP-SL( ', phi,'=2 )(T=5)')),
                 expression(paste('gMAP( ', phi,'=2 )')),
                 expression(paste('DMAP-SA( ', phi,'=log(n) )')),
                 expression(paste('DMAP-SL( ', phi,'=log(n) )(T=5)')),
