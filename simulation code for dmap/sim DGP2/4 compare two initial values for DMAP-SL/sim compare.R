...
result1 = rep_compare_MSPEandTime_fun2(num_rep=200, N=2^12, n_test=100, J=2^5, p=15, 
              p0=200, rho=0.5, sigma0=sqrt((1-0.5)/0.5), T=5, cm_type="nested", n_group=NULL)
re1 = t(cbind(result1$time_mean[1, ], result1$mspe_mean[1,]))
rownames(re1) = c("Time(s)", "MSPE"); re1

## N=2^13, R^2=0.5, J=2^5, p=15 ### 
result2 = rep_compare_MSPEandTime_fun2(num_rep=200, N=2^13, n_test=100, J=2^5, p=15, 
              p0=200, rho=0.5, sigma0=sqrt((1-0.5)/0.5), T=5, cm_type="nested", n_group=NULL)
re2 = t(cbind(result2$time_mean[1, ], result2$mspe_mean[1,]))
rownames(re2) = c("Time(s)", "MSPE"); re2

## N=2^13, R^2=0.5, J=2^6, p=15 ### 
result3 = rep_compare_MSPEandTime_fun2(num_rep=200, N=2^13, n_test=100, J=2^6, p=15, 
              p0=200, rho=0.5, sigma0=sqrt((1-0.5)/0.5), T=5, cm_type="nested", n_group=NULL)
re3 = t(cbind(result3$time_mean[1, ], result3$mspe_mean[1,]))
rownames(re3) = c("Time(s)", "MSPE"); re3

## N=2^13, R^2=0.9, J=2^6, p=15 ### 
result4 = rep_compare_MSPEandTime_fun2(num_rep=200, N=2^13, n_test=100, J=2^6, p=15, 
              p0=200, rho=0.5, sigma0=sqrt((1-0.9)/0.9), T=5, cm_type="nested", n_group=NULL)
re4 = t(cbind(result4$time_mean[1, ], result4$mspe_mean[1,]))
rownames(re4) = c("Time(s)", "MSPE"); re4


## N=2^13, R^2=0.9, J=2^6, p=30 ### 
result5 = rep_compare_MSPEandTime_fun2(num_rep=200, N=2^13, n_test=100, J=2^6, p=30, 
              p0=200, rho=0.5, sigma0=sqrt((1-0.9)/0.9), T=5, cm_type="nested", n_group=NULL)
re5 = t(cbind(result5$time_mean[1, ], result5$mspe_mean[1,]))
rownames(re5) = c("Time(s)", "MSPE"); re5

re = rbind(re1, re2, re3, re4, re5); re 
write.table(re, file="MSPE and time 5 settings(Case 1)(T=5).txt")
