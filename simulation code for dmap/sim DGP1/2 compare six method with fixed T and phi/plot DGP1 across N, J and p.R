...
MSPE_N = read.table("MSPE across N(J=2^6, p=15, R2=0.5, T=3)(Case 1).txt", header=TRUE)
MSPE_Nb = read.table("MSPE across N(J=2^6, p=15, R2=0.5, T=3)(Case 2).txt", header=TRUE)
N_vec = (1:5)*2^12
...
## Case 1, MSPE_N ##
plot(N_vec, log(MSPE_N[,1], 10), ylim=c(0, 0.4), type="n", xaxt="n",  #range(log(MSPE_N, 10))[1]
...
## Case 2, MSPE_N ##
plot(N_vec, log(MSPE_Nb[,1], 10), ylim=c(0, 2), type="n", xaxt="n",  #range(log(MSPE_N, 10))[1]
      xlab="Sample size N", ylab=expression(log[10](MSPE)))
for(i in 1:length(name_method)){
...
## Case 1, MSPE_J ##
plot(J_vec, MSPE_J[,1], ylim=c(1, 1.4), type="n", #range(MSPE_J)[1]
...
## Case 2, MSPE_J ##
plot(J_vec, MSPE_Jb[,1], ylim=c(1, 3), type="n",  #range(MSPE_Jb)[1]
            xlab="Number of machines J", ylab="MSPE")
for(i in 1:length(name_method)){
...
## Case 1, MSPE_p ##
plot(p_vec, MSPE_p[,1], ylim=c(1.0, 2), type="n",
...
RISK_N = read.table("RISK across N(J=2^6, p=15, R2=0.5, T=3)(Case 1).txt", header=TRUE)
RISK_Nb = read.table("RISK across N(J=2^6, p=15, R2=0.5, T=3)(Case 2).txt", header=TRUE)
N_vec = (1:5)*2^12
...
## Case 2, RISK_N ##
plot(N_vec, RISK_Nb[,1], ylim=c(0, 3), type="n", xaxt="n",  #range(log(RISK_N, 10))[1]
      xlab="Sample size N", ylab="RISK")
for(i in 1:length(name_method)){
...
## Case 1, RISK_J ##
plot(J_vec, RISK_J[,1], ylim=c(0, 0.5), type="n", #range(RISK_J)[1]
